# db_llm_executor.py
"""
DB-LLM executor (execution side).
- Accepts analyzer prompt objects built by db_llm_builder.
- Calls the LLM for analysis (to get final_prompt).
- Optionally substitutes placeholders and executes the final_prompt to get parsed results.

Requires:
- steps.ask_llm(prompt) -> str
- steps_new.clean_llm_json(text) -> cleaned JSON string (if available)
"""
