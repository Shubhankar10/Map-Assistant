...
class FederatorLLM:
    def __init__(self, db_llm: list, data: str):
        self.db_llm = db_llm
        self.data = data
    
    def _build_prompt(self):
        prompt = f"""
You are an advanced Reasoning + Data-Enriched LLM module.

### TASKS TO PERFORM (from db_llm)
You must execute the following tasks in order:

{self.db_llm} + {self.data}

### IMPORTANT RULES
- You must use real-world knowledge + reasoning + the provided data.
- You must perform the tasks naturally as a real LLM agent.
- Do NOT use placeholders.
- Your output MUST be complete, factual, and directly actionable.
- Unless any db_llm instruction demands more:
      → RETURN ONLY limited but all and the most relevent KEYWORDS extracted from your analysis.
      - what every is the most reuired one word answer, return that first, then in order.
- No extra sentences.
- No explanations.
- No formatting other than raw JSON list of keywords.
- Make sure all data is translated to english.

### OUTPUT FORMAT
Return only a list of keywords, like:
["keyword1", "keyword2"]
"""
    ...
    def run(self):
        ...
        if not isinstance(response, list):
            raise TypeError("FederatorLLM.run() expected a list of keywords.")

        return [str(k).strip() for k in response if k]
...
if __name__ == "__main__":
    ...
    # Sample input
    sample_db_llm = [
        "Find capitals of the states where each spoken language is native."
    ]

    sample_data = """
Languages: Kannada, Gujrati, French
"""

    federator = FederatorLLM(
        db_llm=sample_db_llm,
        data=sample_data
    )
